...
export default function WorkSection() {
  ...
  const [modalPosition, setModalPosition] = useState<ProjectPosition | null>(null);
  ...
  const handleProjectClick = (detailId: number, event: MouseEvent<HTMLButtonElement>) => {
    const element = projectRefs.current[detailId];
    if (element) {
      ...
      const projectDetail = projectDetails.find((p) => p.id === detailId);
      if (projectDetail) {
  ...
  return (
    <section id="work" className="py-20 bg-transparent">
      {selectedProject && (
        <ProjectModal project={selectedProject} onClose={handleCloseModal} modalPosition={modalPosition} />
      )}
      ...
      <div className="container">
        ...
        >
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-6xl transition-opacity duration-300 ${selectedProject ? 'opacity-30' : 'opacity-100'}`}>
