    ...
    <section className="w-full bg-marquee rounded-t-[20px]">
      <div className="w-full flex justify-between gap-y-[50px] padding-x pb-[50px] flex-wrap">
        <div className="w-[49%]" key={item.id}>
          <div className="flex gap-x-[10px] items-center pb-[10px]">
            ...
            <h1 className="text-[18px] leading-[21px] uppercase font-medium font-Bricolage text-white">{item.title}</h1>
          </div>
            ...
            >
              {item.title.split("").map((item: any, i: number) => (
                <motion.span
                  ...
                  className="text-[165px] leading-none inline-block uppercase font-Bricolage text-about font-bold text-center"
                  key={i}
                >
                  {item}
                </motion.span>
          ...
          <div className="flex items-center gap-[10px] mt-[20px] flex-wrap">
            {item.links.map((link: any) => (
              <div className="w-fit rounded-[50px] border border-[#ffffff77] cursor-pointer" key={link.id}>
                <Link
                  className="text-[18px] leading-[18px] font-Bricolage text-white uppercase group-hover:text-secondry transition-all duration-300 ease-in-out hover:text-secondry"
                  href={link.href}
                >
      ...
      <div className="w-full flex pt-[50px] pb-[200px] justify-center">
        <div className="flex items-center justify-between bg-white cursor-pointer rounded-full group">
          <Link className="text-[19px] text-secondry uppercase font-normal font-Bricolage" href={"/presentation"}>
